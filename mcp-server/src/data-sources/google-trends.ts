...
export class GoogleTrendsDataSource {
  private baseUrl = 'https://trends.google.com/trends/api/widgetdata/multiline';

  async fetchTrends(keywords: string[]): Promise<{ [keyword: string]: number }> {
    ...
    try {
      for (const keyword of keywords) {
        ...
        if (parsedData.timelineData && parsedData.timelineData.length > 0) {
          ...
          const avgValue = recentData.reduce((sum: number, point: any) => 
            sum + (point.value[0] || 0), 0) / recentData.length;
